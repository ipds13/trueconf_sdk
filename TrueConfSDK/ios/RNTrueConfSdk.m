...
@interface RNTrueConfSdk() <TCConfControlsDelegate, TCWindowsDelegate>
...
@property (strong, nonatomic) UIView* xsview;
...
RCT_EXPORT_METHOD(start:(NSString *) serverList)
{
    if(!self.tcsdk) {
        ...
        self.tcsdk = [[TCSDK alloc] initWithViewController:rootController forServer:@"qa4.trueconf.net"];
        self.tcsdk.trueConfSDKLogEnable = YES;
...
RCT_EXPORT_METHOD(initCustomViews)
{
    ...
    self.xsview.backgroundColor = [UIColor yellowColor];
    [self.xsview.bottomAnchor constraintEqualToAnchor:rootView.bottomAnchor constant: 0].active = YES;
    ...
    self.xview.backgroundColor = [UIColor greenColor];
    [self.xview.bottomAnchor constraintEqualToAnchor:rootView.bottomAnchor constant: 0].active = YES;
...
RCT_EXPORT_METHOD(addExtraButton:(NSString*)title resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject)
{
    @try
    {
        NSMutableArray<UIAlertAction *> *btns = [[NSMutableArray alloc] init];
        UIAlertAction* btn = [UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if(hasListener) {
